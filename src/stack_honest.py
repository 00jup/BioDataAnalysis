...
CHEMPROP_V17 = os.path.join(
    PROJECT_ROOT, "models", "chemprop_scaffold_v2", "v17_ens15_h600", "vivo"
)
...
def main():
    ...
    # val + test 가져오기
    base = os.path.join(PROJECT_ROOT, "models", "chemprop_scaffold_v2", "v12_baseline", "vivo")
    all_csv = pd.read_csv(
        os.path.join(PROJECT_ROOT, "data", "chemprop_scaffold_v2", "vivo", "all.csv")
    )

    val_smi = pd.read_csv(f"{base}/val_smiles.csv").merge(
        all_csv, on="canonical_smiles", how="left"
    )
    test_smi = pd.read_csv(f"{base}/test_smiles.csv").merge(
        all_csv, on="canonical_smiles", how="left"
    )
    ...
    for nm, pv, pt in [("Chemprop", val_p_cp, test_p_cp), ("RF/CB v2", val_p_rf, test_p_rf)]:
        # val 에서 thr 결정
        bt, bm = 0.5, -1.0
