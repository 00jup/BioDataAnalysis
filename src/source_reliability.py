...
SOURCE_DEFS = [
    ("dilirank_vMost",   "vivo_dilirank", lambda v: 1 if v == "vMost-DILI-Concern" else (None if pd.isna(v) else 0)),
    ("dilirank_vLess",   "vivo_dilirank", lambda v: 1 if v == "vLess-DILI-Concern" else (None if pd.isna(v) else 0)),
    ("dilirank_vNo",     "vivo_dilirank", lambda v: 0 if v == "vNo-DILI-Concern" else (None if pd.isna(v) else 1)),
    ("livertox_A_B",     "vivo_livertox", lambda v: 1 if v in ("A","B") else (0 if v == "E" else None)),
    ("livertox_C_D",     "vivo_livertox", lambda v: 1 if v in ("C","D") else (0 if v == "E" else None)),
    ("livertox_E",       "vivo_livertox", lambda v: 0 if v == "E" else (1 if v in ("A","B","C","D") else None)),
    ...
    ("marketed_clean_neg","vivo_marketed_clean_neg", lambda v: 0 if v == 1 else None),  # 음성 신호만
...
# 방법 1: Heuristic
# ════════════════════════════════════════════════════════════════
def pairwise_agreement(labels_by_source: dict, reference: str = "dilirank_vMost") -> dict[str, dict]:
...
def classifier_auc(db: pd.DataFrame, labels_by_source: dict, gold_source: str = "dilirank_vMost") -> dict[str, dict]:
    ...
    gold_labels = {}
    for i in gold_idx:
        if pd.notna(labels_by_source[gold_source].iloc[i]) and labels_by_source[gold_source].iloc[i] == 1:
            gold_labels[i] = 1
        elif pd.notna(labels_by_source["dilirank_vNo"].iloc[i]) and labels_by_source["dilirank_vNo"].iloc[i] == 0:
            gold_labels[i] = 0
    gold_X = []
    gold_y = []
    gold_smiles = set()
    for i, y in gold_labels.items():
        ...
        if smi in gold_smiles: continue
        gold_smiles.add(smi)
        fp = morgan_fp(smi)
        if fp is None: continue
        gold_X.append(fp); gold_y.append(y)
    gold_X = np.array(gold_X); gold_y = np.array(gold_y)
    print(f"  Gold test: {len(gold_X)} 분자 (양성 {int(gold_y.sum())}, 음성 {int((1-gold_y).sum())})")

    out = {}
    for name, lab in labels_by_source.items():
        if name == gold_source: continue
        ...
        for smi, y in zip(train_smi, train_y):
            fp = morgan_fp(smi)
            if fp is not None:
                Xtr.append(fp); ytr_keep.append(y)
        Xtr = np.array(Xtr); ytr_keep = np.array(ytr_keep)
        ...
        try:
            ...
            auc = None
        out[name] = {"n_train": int(len(ytr_keep)), "n_pos": int(ytr_keep.sum()),
                      "n_neg": int((1-ytr_keep).sum()), "auc": auc}
    return out
...
def feature_based_reliability(db: pd.DataFrame, labels_by_source: dict) -> dict[str, dict]:
    ...
    for target in labels_by_source:
        ...
        for i in range(len(db)):
            ...
            if len(votes) >= 2:
                n_pos = sum(votes); n_neg = len(votes) - n_pos
                if n_pos > n_neg:
                    consensus_label.append(1); n_agreed.append(len(votes))
                elif n_neg > n_pos:
                    consensus_label.append(0); n_agreed.append(len(votes))
                else:
                    consensus_label.append(None); n_agreed.append(0)
            else:
                consensus_label.append(None); n_agreed.append(0)
...
def main():
    ...
    for name, lab in labels.items():
        print(f"  {name:22s} {int(lab.notna().sum()):5d} (양성 {int((lab==1).sum()):4d} / 음성 {int((lab==0).sum()):4d})")
    ...
    for name, m in sorted(fb.items(), key=lambda x: x[1].get("agreement") or 0, reverse=True):
        a = m.get("agreement"); auc = m.get("auc_against_target_labels")
        print(f"  {name:22s} train_consensus={m['n_train_consensus']:5d}  target={m.get('n_target_labeled', 0):5d}  agreement={'  —' if a is None else f'{a:.3f}'}  AUC={'  —' if auc is None else f'{auc:.3f}'}")
    ...
    for name in labels:
        ...
        vals = []
        if a1 is not None: vals.append(a1)
        if b1 is not None: vals.append((b1 - 0.5) * 2)
        if f2 is not None: vals.append(f2)
    ...
    for name, m in sorted(summary.items(), key=lambda x: x[1].get("final_weight") or 0, reverse=True):
        w = m["final_weight"]
        w_str = "  —" if w is None else f"{w:.3f}"
        print(f"  {name:22s} weight={w_str}  (agr={_fmt(m['agreement_DILIrank_gold'])}, auc={_fmt(m['classifier_auc'])}, fb={_fmt(m['feature_truth_agreement'])})")

    out = {"label_counts": {n: int(lab.notna().sum()) for n, lab in labels.items()},
           "pairwise_agreement": pa, "classifier_auc": ca, "feature_based": fb,
