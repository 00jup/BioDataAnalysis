"""Chemprop v18 — strict label + v17 hp + class_balance.

데이터:
  data/chemprop_scaffold_strict/vivo/all.csv (1163 양성 / 12910 음성)

목적: 진짜 강한 DILI 만 양성으로 학습 → 안전한 약은 음성으로 잘 분류.
v17 (sensitive) 와 비교 — TPR vs TNR 균형.
"""
...
DATA_DIR = os.path.join(PROJECT_ROOT, "data", "chemprop_scaffold_strict")
MODELS_DIR = os.path.join(PROJECT_ROOT, "models", "chemprop_strict_v18")
RESULTS = os.path.join(PROJECT_ROOT, "results")
CHEMPROP_BIN = os.path.join(os.path.dirname(sys.executable), "chemprop")


def train_vivo():
    print(f"\n=== Chemprop v18 strict — vivo ===")
    save_dir = os.path.join(MODELS_DIR, "vivo"); os.makedirs(save_dir, exist_ok=True)
    ...
    cmd = [
        ...
        "--class-balance",                    # imbalance 1:11 — 균형 필요
        "--ensemble-size", "15",
    ...
    t0 = time.time()
    print(f"  설정: strict label + class_balance + ensemble 15 + hidden 600")
    ...
    print(f"\n  [vivo v18 strict scaffold test] N={len(y)} (양성 {y.sum()})")
...
def main():
    ...
    out = {"vivo": train_vivo()}
    with open(os.path.join(RESULTS, "chemprop_v18_strict.json"), "w") as f:
        json.dump(out, f, indent=2)
    print(f"\n저장: results/chemprop_v18_strict.json")
