...
def train(domain):
    ...
    save_dir = os.path.join(MODELS_DIR, domain); os.makedirs(save_dir, exist_ok=True)
    splits_file = os.path.join(PROJECT_ROOT, "models", "chemprop_scaffold_v2",
                                "v12_baseline", domain, "splits.json")
    ...
    test_csv = os.path.join(PROJECT_ROOT, "models", "chemprop_scaffold_v2",
                             "v12_baseline", domain, "test_smiles.csv")
    ...
    print(f"  [{domain} v17] AUC {auc:.3f}  MCC {bm:.3f}  TPR {out['tpr']:.3f}  TNR {out['tnr']:.3f}")
    return out
