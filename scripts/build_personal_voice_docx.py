"""격식 학술 한국어 보고서 — [숫자] 인용, 본문 등장 순서로 reference 번호 매김."""
...
def build_doc():
    ...
    add_para(doc,
        "초기 모델 구축은 가장 간단한 baseline 으로부터 시작하였다. PubChem 에서 약물의 SMILES [1] 를 수집한 후, "
        "RDKit 라이브러리를 사용하여 ECFP6 fingerprint [2] 를 계산하고 RandomForest [3] 에 입력하였다. "
    ...
    add_para(doc,
        ...
        "관련 문헌 검토 결과 MoleculeNet 연구 [4] 와 Chemprop 연구 [5] 에서 권장하는 Bemis-Murcko scaffold split "
        "[6] 을 채택하였다.")
    ...
    add_para(doc,
        ...
        "추가 수집하였다. 초기에 확보한 source 는 FDA DILIrank [7] 와 NIH LiverTox [8] 였으나, 각각 1,036 / 851 "
        "분자 규모로 학습에 충분하지 않았다.")

    add_para(doc,
        "따라서 다음의 8개 source 를 통합하였다: DILIrank, LiverTox, FDA DailyMed (의약품 라벨 기반 hepatic AE), "
        "PubMed (DILI 관련 문헌의 약물 mention), Comparative Toxicogenomics Database [9], FDA Adverse Event "
        "Reporting System (FAERS) [10], ChEMBL [11] 의 max_phase = 4 (FDA approved) 약물, 그리고 post-2010 출시 + "
        "DILI 보고 없는 약물의 음성 anchor (Marketed Clean). 8개 source 의 분자 수 단순 합은 39,087개이나 source 간 "
        "중복이 존재하여 InChIKey 기준 중복 제거 후 vivo labeled 13,777개 (양성 3,393 / 음성 10,384) 가 최종 학습 "
        "데이터로 확보되었다. Source 별 상세 분포는 Table 1 에 제시한다.")
    ...
    add_table_from_rows(doc, [
        ...
        ["단순 합 (source 간 중복 포함)", "—", "39,087", "8,068", "30,519"],
        ["통합 (InChIKey dedup, vivo 학습 사용)", "—", "13,777", "3,393", "10,384"],
    ])
    add_table_caption(doc,
        "Table 1. 통합한 8개 DILI source 의 분포. 8개 source 의 단순 합은 39,087 분자이나, source 간 중복이 존재하여 "
        "InChIKey 기준 중복 제거 및 RDKit standardization 후 vivo labeled 13,777개 (양성 3,393 / 음성 10,384) 가 "
        "최종 학습 데이터로 확보되었다. Vitro (in vitro 세포 기반 assay) 라벨은 학습에서 제외하였다. 신뢰도 검토 후 "
        "제외한 source 는 Gold standard, DILIst, SIDER, TDC DILI, ClinTox 의 5개이다."
    )
    ...
    add_para(doc,
        ...
        "InChIKey [12] 를 분자의 고유 식별자로 사용하여 중복을 제거하였다.")
    ...
    add_para(doc,
        ...
        "hepatitis 로 FDA 가 회수한 약물이다 [13]. Acetaminophen 의 경우 분자 자체는 안전하나 활성 대사산물인 NAPQI 가 "
        "간세포의 glutathione 을 고갈시키며 손상을 유발한다 [14]. 이러한 mismatch 분자가 학습 데이터에 포함될 경우 "
        "노이즈로 작용함이 ablation 에서 확인되었다.")

    add_para(doc,
        "따라서 임상 의의가 직접적인 in vivo 라벨 13,777개 (양성 3,393 / 음성 10,384) 만을 학습에 사용하였다. "
        "In vitro labeled 분자는 학습 데이터에서 완전히 제외하였다.")

    # 2.3
    add_heading(doc, "2.3 Feature 선택", level=2)

    add_para(doc,
        "Feature 의 선택은 단계적으로 진행하였다. 초기에는 ECFP6 fingerprint [2] 단독으로 시작하였다. ECFP6 은 각 "
        "원자 주변의 부분 구조를 hash 하여 2,048 bit 의 binary vector 로 표현하는 방식이다. RandomForest 에 입력한 "
        "결과 scaffold OOD MCC 0.55 의 성능을 얻었다.")

    add_para(doc,
        "다양한 분자 정보를 활용하기 위해 5종 fingerprint 로 확장하였다: ECFP6, Avalon [15], AtomPair [16], "
        "Topological Torsion [17], Pattern (MACCS-like) [18]. RandomForest 와 CatBoost [19] 의 ensemble 로 학습한 "
        "결과 MCC 0.59 로 향상되었다.")

    add_para(doc,
        "Fingerprint 기반 학습은 분자의 그래프 구조 정보를 1차원 vector 로 펴면서 정보 손실이 발생할 가능성이 있다고 "
        "판단하였다. 이에 그래프 신경망인 Chemprop [5, 20] 의 D-MPNN (Directed Message Passing Neural Network) 을 "
    ...
    add_para(doc,
        "추가로 200차원 RDKit 2D descriptor (분자량, LogP, TPSA [21], H-bond donor / acceptor, 회전 가능 결합 수, "
        "ring 수 등) 를 Chemprop 의 graph representation 에 concat 하였다. Chen et al. [22] 의 'Rule of Two' 연구에서 "
    ...
    add_para(doc,
        "Section 2.1 에서 언급한 random split 의 한계를 해결하기 위해 Bemis-Murcko scaffold-balanced split [6] 을 "
    ...
    add_para(doc,
        ...
        "benchmark [4] 의 표준 설정을 기반으로 결정하였다. Ensemble size 5 의 경우 prediction variance 가 큰 것으로 "
    ...
    add_para(doc,
        ...
        "비교한 결과, focal loss [23] (γ=2) 와 SMOTE oversampling [24] 모두 scaffold OOD MCC 가 각각 -0.007, -0.003 의 "
    ...
    add_para(doc,
        ...
        "Specificity (TNR), Matthews Correlation Coefficient (MCC) [25], Area Under ROC Curve (AUC) [26] 의 5가지를 "
        "사용하였다. 데이터 imbalance 환경에서 가장 신뢰할 수 있는 단일 지표인 MCC [27] 를 주요 평가 지표로 채택하였다.")
    ...
    add_para(doc,
        ...
        "Chemprop + RF/CB 의 linear stacking 은 MCC 0.617 로 단일 Chemprop 대비 +0.002 의 marginal 향상에 "
    ...
    add_para(doc,
        ...
        "DILI ML 모델을 검토한 결과, MoLFormer [28], ChemBERTa [29], GROVER [30], KPGT [31] 등 최신 분자 표현 학습 "
        "모델들도 외부 신약 평가에서 유사한 한계를 보임이 확인되었다 [32, 33]. 본 한계는 특정 모델의 문제가 아니라 "
    ...
    add_para(doc,
        "두 그룹을 분리하여 Chemprop v27 의 성능을 측정한 결과는 Table 6 과 같다.")
    ...
    add_para(doc,
        "DILI 예측의 본질적 어려움은 다음의 세 가지 요인으로 분석할 수 있다 [34].")

    add_para(doc,
        "첫째, DILI 의 특이체질 (idiosyncratic) 본질이다 [35]. 환자마다 발생 여부가 상이하며, 분자 구조만으로는 "
        "결정되지 않는다. (a) 환자 HLA (인간 백혈구 항원) 유전형의 영향이 보고되었다. 예를 들어 HLA-B*5701 + "
        "flucloxacillin 의 조합에서만 cholestatic hepatitis 가 발생함이 GWAS 로 확인된 바 있다 [36, 37]. (b) 대사산물 "
        "매개 독성이 존재한다. Acetaminophen 의 경우 약물 자체는 안전하나 활성 대사산물 NAPQI 가 독성을 유발한다 [14]. "
        "(c) 발생률이 1,000명 ~ 10,000명 당 1명 수준으로 매우 낮아 통계적 학습 신호가 약하다 [38].")
    ...
    add_para(doc,
        ...
        "(3) Focal loss [23] (γ=2) — scaffold OOD MCC -0.007. (4) SMOTE oversampling [24] — scaffold OOD MCC -0.003. "
        "(5) Chemprop + RF/CB 의 linear stacking — 단일 Chemprop 대비 +0.002 MCC, 코드 복잡도 증가 대비 효과 미미하여 "
        "production 에서 제외.")

    add_heading(doc, "3.4 향후 연구 방향", level=2)

    add_para(doc,
        "DILI 예측의 본질적 한계를 점진적으로 극복하기 위한 향후 연구 방향은 다음과 같다.")

    add_para(doc,
        "첫째, 분자 표현의 사전학습 (pre-training) 활용이다. 본 연구는 19,273개의 DILI labeled 분자만을 사용하였으나, "
        "MoLFormer [28] (IBM, 11억 분자 사전학습) 등의 분자 foundation model 을 활용하면 모델이 일반적 화학 지식을 "
        "사전에 학습한 상태에서 DILI 데이터로 fine-tune 할 수 있다. 학계 보고에 따르면 외부 신약 예측 AUC 가 "
        "0.50 → 0.55~0.62 정도 향상 가능하다 [32].")
    ...
    add_para(doc,
        "셋째, 능동 학습 (active learning) 의 적용이다 [39]. 모델이 불확실한 예측을 보이는 신약 (predict_proba "
    ...
    add_para(doc,
        ...
        "함으로써 novel chemical class 분자에 대해 예측 신뢰도가 낮음을 명시할 수 있다. Monte Carlo dropout [40] "
        "또는 ensemble disagreement 기반 방법 [41] 을 활용할 수 있다.")
    ...
    add_para(doc,
        "본 모델은 학습 분포 내 신약에 대해 의미 있는 일반화 성능 (MCC 0.615) 을 제공한다. 다만 novel chemical class "
        "에 대해서는 random 수준의 한계가 존재함을 본 연구에서 확인하였으며, 이러한 한계를 보고서에 명시하였다. "
        "향후 foundation model 및 multi-modal 통합을 통해 외부 OOD 신약 예측의 한계를 점진적으로 극복할 수 있을 것으로 "
        "기대된다.")

    # References — 본문 등장 순서대로
